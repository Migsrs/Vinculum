...
import { isHashed } from "./utils/security";
...
function App() {
  ...
  useEffect(() => {
    ...
    // Usuários: mescla seeds + migra senhas em texto puro para hash
    const storedUsers = readLS(LS_KEYS.users, []);
    const seedPasswordMap = Object.fromEntries(
      seedUsers.map((u) => [u.email, u.password])
    );
    const existingEmails = new Set(storedUsers.map((u) => u.email));

    // Migra senhas não-hasheadas de usuários seed já no localStorage
    const migratedUsers = storedUsers.map((u) => {
      if (seedPasswordMap[u.email] && !isHashed(u.password)) {
        return { ...u, password: seedPasswordMap[u.email] };
      }
      return u;
    });

    const missingUsers = seedUsers.filter((u) => !existingEmails.has(u.email));
    const finalUsers = missingUsers.length > 0
      ? [...migratedUsers, ...missingUsers]
      : migratedUsers;

    // Só escreve se algo mudou
    if (
      missingUsers.length > 0 ||
      migratedUsers.some((u, i) => u !== storedUsers[i])
    ) {
      writeLS(LS_KEYS.users, finalUsers);
    }
  }, []);
